...
function processSum(numberList, cb) {
  const sum = numberList.reduce((x, y) => { return x + y }, 0);
  return cb(sum);
}
 ...
 * should return 1000.
*/
function processProduct(a, b, cb) {
  return cb(a * b);
}
...
*/
function getFullNames(listOfRunners) {
  const fullNames = [];
...
function tallyUpDonations(runners) {
  return runners.reduce((totalDonations, runner) => { return totalDonations += runner.donation }, 0);
}
